---
name: darwin-skill
description: "Use when the user asks to optimize, review, score, test, or improve an Agent Skill or SKILL.md; mentions 达尔文, Darwin, skill评分, skill质量检查, 自动优化, skill review, skill打分, test prompts, dry-run validation, runtime neutrality, results.tsv, or skill成果卡片."
---
...
```
1. 确认优化范围：
   - 全部skills → 扫描当前 runtime 暴露的 skills 根目录或用户指定的项目级 skills 目录
   - 指定skills → 用户指定列表
2. 如用户要求长期记录或提交变更，创建 git 分支：auto-optimize/YYYYMMDD-HHMM；否则只在当前工作树内编辑并报告
3. 在 Darwin skill 根目录初始化 results.tsv（如不存在）
...
文件位置：Darwin skill 根目录下的 `results.tsv`，即与本 `SKILL.md` 同级。不要硬编码 `.claude`、`.codex`、`.cursor` 等单一 runtime 路径。
